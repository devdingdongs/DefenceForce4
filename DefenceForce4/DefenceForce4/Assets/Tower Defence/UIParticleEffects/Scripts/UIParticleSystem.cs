...
public class UIParticleSystem : MonoBehaviour {
    ...
    private void Init()
    {
        ...
        for (int i = 0; i < ParticlePool.Length; i++)
        {
            ...
            ParticlePool[i].sprite = Particle[UnityEngine.Random.Range(0,Particle.Length)];
        }
    }
    ...
    public void Stop()
    {
        try
        {
            ...
            if(ParticlePool != null)
            {
                for (int i = 0; i < ParticlePool.Length; i++)
                {
                    if (ParticlePool[i].gameObject != null)
        ...
        {
            Debug.Log("stop particle : " + e.Message);
        }
    }
    ...
    private IEnumerator CoParticleFly(Image particle)
    {
        ...
        //apply angle
        emissonAngle = Quaternion.AngleAxis(UnityEngine.Random.Range(-EmissionAngle / 2f, EmissionAngle / 2f), Vector3.forward) * emissonAngle;
        //normalize
